...
# read protein ID to gene ID matching files and remove duplicate rows (originate from multiple transcritps)
prot_gene_matching <- unique(data.table::fread(paste0(cfg$output_path,'tmp/tmp.prot_gene_matching.txt'), sep="\t", header=FALSE, stringsAsFactors = FALSE)[,1:2])
...
if (length(genes_coords_taxon$best_hitID[!is.na(genes_coords_taxon$best_hitID) & genes_coords_taxon$LCA_replace == TRUE]) > 0) {
    ...
    # intersection of lineage of query species and best hit
    # yields the ID of LCA of best hit and query species (=corrected LCA), else NA
    corrected_lcaID <- lapply(best_hit_lineage, function(x) (intersect(x,query_lineage))[1])
    # get fullName for corrected LCA ID
    # variable 'counter' enables to iterate through lists in 'best_hits_taxonomy' and rows in 'corrected_lcaID' at the same time
    # thereby you can get the fullName of corrected LCA in the corresponding taxonomy list in best_hits_taxonomy
    # if corrected_lcaID == NA, this returns multiple NAs; hence only the first element returned is picked (last [1])
    counter <- 0
    corrected_lca <- lapply(best_hits_taxonomy, function(x) {counter <<- counter + 1; x$fullName[x$ncbiID == corrected_lcaID[counter][1]][1]})
    ...
    genes_coords_taxon <- merge(genes_coords_taxon, label_names, by = "taxon_assignmentID", all.x = TRUE)
        ...
        genes_coords_taxon$corrected_lcaID <- NA
        genes_coords_taxon$corrected_lca <- NA
        genes_coords_taxon$taxon_assignmentID <- genes_coords_taxon$lcaID
        genes_coords_taxon$taxon_assignment <- genes_coords_taxon$lca_hit
}
...
if (cfg$num_groups_plot != "all") {
    ...
    max_num_groups <- as.numeric(cfg$num_groups_plot)-1 # maximal number of groups to display in plot
    taxons_keep <- taxon_count_sort[1:max_num_groups,"Var1"]
    # genes with taxonomic assignemnt of low abundance get plot_label "Otherwise assigned"
    genes_coords_taxon$plot_label[! genes_coords_taxon$plot_label %in% taxons_keep] <- "Otherwise assigned"
}
...
default_cols_1 <- c("g_name","c_name","c_num_of_genes","c_len","c_pct_assemby_len","c_genelenm","c_genelensd") # columns that every genes_coords_taxon table has part1
default_cols_2 <- c("c_pearson_r","c_pearson_p","c_gc_cont","c_gcdev","g_len","g_lendev_c","g_lendev_o","g_abspos","g_terminal","g_single") # columns that every genes_coords_taxon table has part2
default_cols_3 <- c("g_pearson_r_o","g_pearson_p_o","g_pearson_r_c","g_pearson_p_c","g_gc_cont","g_gcdev_c","g_gcdev_o") # columns that every genes_coords_taxon table has part3

c_cov_cols <- grep("c_cov|c_genecov",colnames(genes_coords_taxon),value=TRUE) # columns with c_cov values
g_cov_cols <- grep("g_cov",colnames(genes_coords_taxon),value=TRUE) # columns with g_cov values
dim_cols <- grep("Dim.",colnames(genes_coords_taxon),value=TRUE) # columns with Dim values (coordinates)
tax_columns <- c("protID","lcaID","lca_hit","score","best_hitID","best_hit","evalue","corrected_lcaID","corrected_lca","taxon_assignmentID","taxon_assignment","plot_label") #  taxonomy assignment related columns

cols <- c(default_cols_1,c_cov_cols,default_cols_2,g_cov_cols,default_cols_3,dim_cols,tax_columns)

raw_gene_table_orphan_info <- data.frame(genes_coords_taxon)[,cols]
write.csv(raw_gene_table_orphan_info, file=paste0(cfg$output_path, "taxonomic_assignment/gene_table_taxon_assignment.csv"), row.names=FALSE, quote=FALSE)
