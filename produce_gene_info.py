...
def impute_array(a, data_array, include_coverage):
    ...
    c_genecovsds = [[] for pbc_index in range(a.get_num_pbc())]
    c_genelensds = []

    g_covdev_cs = [[] for pbc_index in range(a.get_num_pbc())]
    g_lendev_cs = []
    g_gcdev_cs = []
    ...
    my_mean1 = [np.nanmean(coverages) if coverages else np.nan for coverages in c_genecovsds]
    my_mean2 = np.nanmean(c_genelensds) if g_lendev_cs else np.nan
    my_mean3 = [np.nanmean(coverages) if coverages else np.nan for coverages in g_covdev_cs]
...
def get_sum_of_contig_lengths(a):
    ...
    for contig_name in a.get_contigs().keys():
...
def compute_coverage_and_positional_info(a):
    ...
    for contig_name, contig in a.get_contigs().items():
        ...
        if contig.geneless_info() == 1:
            contig.set_gene_coverage_mean_and_sd([np.nan for coverages in range(a.get_num_pbc())], [np.nan for coverages in range(a.get_num_pbc())])
            ...
            for pbc_index in range(a.get_num_pbc()): #for each coverage file
            ...
            if len(gene_names) == 1:
                ...
                contig.set_gene_coverage_mean_and_sd(contig_weighted_gene_cov_mean,
                                                 [np.nan for pbc_index in range(a.get_num_pbc())]) # set standard deviation to NaN
            ...
            # if there is more than one gene, also set standard deviation
                ...
                contig.set_gene_lengths_mean_and_sd(np.mean(contig_gene_lengths), #TODO: np.nan?
...
def filter_contigs_and_genes(a):
    ...
    for contig_name, contig in a.get_contigs().items():
        ...
        if contig.geneless_info() == 1:
            a.add_geneless_contig(contig_name)
...
def get_raw_array(a, stats_ref):
    ...
    for contig_name in a.get_contigs().keys():
        ...
        if c_ngenes > 0:
            ...
            c_covdev = [contig.covdev_from_overall(stats_ref["contig cov mean"][pbc_index], stats_ref["contig cov sd"][pbc_index], pbc_index) for pbc_index in range(a.get_num_pbc())]
...
def output_summary_file(a, stats_ref, filename):
...
def main():
    ...
    output_summary_file(a, stats_for_all_contigs, "summary.txt")
